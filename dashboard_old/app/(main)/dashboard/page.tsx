...
import { BookOpen, CheckCircle, ChevronLeft, ChevronRight, Cpu, Database, Edit, Eye, Globe, PenTool, Search, Server, Users } from "lucide-react"
  ...
  const fetchMetrics = async () => {
    try {
      ...
      setStats((prev) => ({
        ...
        // Mock host metrics
        hostCpu: Math.floor(Math.random() * 30) + 10, // 10-40%
        hostMemory: Math.floor(Math.random() * 500) + 200, // 200-700 MB
        hostDisk: Math.floor(Math.random() * 20) + 40, // 40-60%
      }))
  ...
  return (
    <div className="min-h-screen text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {/* Row 1: System Stats */}
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <div className="text-sm text-white/60">Memory Records</div>
              <div className="text-2xl font-bold text-white">{stats.memoryRecords.toLocaleString()}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <div className="text-sm text-white/60">Top Writer</div>
              <div className="text-xl font-bold text-white truncate max-w-[150px]" title={stats.topWriter.name}>
                {stats.topWriter.count > 0 ? stats.topWriter.name : "N/A"}
              </div>
              <div className="text-xs text-white/40">
                {stats.topWriter.count > 0 ? `${stats.topWriter.count} writes` : "No activity"}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              <div className="text-sm text-white/60">Top Reader</div>
              <div className="text-xl font-bold text-white truncate max-w-[150px]" title={stats.topReader.name}>
                {stats.topReader.count > 0 ? stats.topReader.name : "N/A"}
              </div>
              <div className="text-xs text-white/40">
                {stats.topReader.count > 0 ? `${stats.topReader.count} reads` : "No activity"}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div className="overflow-hidden">
              <div className="text-sm text-white/60">Last Writer</div>
              <div className="text-xl font-bold text-white truncate max-w-[150px]" title={stats.lastWriter.name}>
                {stats.lastWriter.timestamp > 0 ? getClientDisplayName(stats.lastWriter.name) : "N/A"}
              </div>
              <div className="text-xs text-white/40">
                {stats.lastWriter.timestamp > 0 ? new Date(stats.lastWriter.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'}) : "No activity"}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div className="overflow-hidden">
              <div className="text-sm text-white/60">Last Reader</div>
              <div className="text-xl font-bold text-white truncate max-w-[150px]" title={stats.lastReader.name}>
                {stats.lastReader.timestamp > 0 ? getClientDisplayName(stats.lastReader.name) : "N/A"}
              </div>
              <div className="text-xs text-white/40">
                {stats.lastReader.timestamp > 0 ? new Date(stats.lastReader.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'}) : "No activity"}
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <ReactECharts
                option={{
                  ...
                  series: clientNames.map((client) => ({
                    ...
                    areaStyle: {
                      color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                        { offset: 0, color: hexToRgba(getClientColor(client, clientNames), 0.1) },
                        { offset: 1, color: hexToRgba(getClientColor(client, clientNames), 0) }
                      ]),
                      opacity: 1
                    }
                  })),
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                <div className="text-sm text-white/60">Host CPU Load</div>
                <div className="text-2xl font-bold text-white">{stats.hostCpu}%</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                <div className="text-sm text-white/60">Host Memory Usage</div>
                <div className="text-2xl font-bold text-white">{stats.hostMemory} MB</div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/10 backdrop-blur-md border-white/10 shadow-lg">
            <CardContent className="p-6 flex items-center gap-4">
              ...
              <div>
                <div className="text-sm text-white/60">Host Disk Space Used</div>
                <div className="text-2xl font-bold text-white">{stats.hostDisk}%</div>
              </div>
            </CardContent>
          </Card>
        </div>
