...
export async function getSuccessRate(duration: string = '15m'): Promise<number> {
  // Calculate success rate over the specific duration using increase()
  // This ensures we only count requests that happened within the selected time window
  const result = await query(`sum(increase(openmemory_requests_total{status=~"2..|3.."}[${duration}])) / sum(increase(openmemory_requests_total[${duration}])) * 100`)
...
export async function getSuccessRateByClient(duration: string = '15m'): Promise<Record<string, number>> {
  ...
  const [totalResult, successResult] = await Promise.all([
    query(`sum by (client_name) (increase(openmemory_requests_total[${duration}]))`),
    query(`sum by (client_name) (increase(openmemory_requests_total{status=~"2..|3.."}[${duration}]))`)
  ])
...
export async function getSuccessRateTimeSeriesByClient(duration: string = '1h'): Promise<Array<{ time: number, [client: string]: number | string }>> {
  ...
  // Use rate() over a 5m window to show the success rate "at that moment"
  // This prevents the graph from being cumulative or flat 100% if using raw counters
  const [allClients, successRateResult] = await Promise.all([
    getAllActiveClients(duration),
    queryRange(
      `sum by (client_name) (rate(openmemory_requests_total{status=~"2..|3.."}[5m])) / sum by (client_name) (rate(openmemory_requests_total[5m])) * 100`,
      start,
      now,
      step
    )
  ...
  for (let t = start; t <= now; t += stepSeconds) {
    ...
    allClients.forEach(client => {
      timeMap.get(t)![client] = 100 // Default to 100% if no data
    })
  }

  successRateResult.data.result.forEach((series) => {
    const clientId = series.metric.client_name || 'unknown'
    series.values?.forEach(([timestamp, value]) => {
      ...
      if (timeMap.has(targetTs)) {
        ...
        // If NaN (div by zero), assume 100% (no errors)
        timeMap.get(targetTs)![clientId] = isNaN(val) ? 100 : val
      }
    })
  })
...
export async function getTopWriter(duration: string = '15m'): Promise<{ name: string, count: number }> {
  // Get client with most write requests in the given duration
  const result = await query(`topk(1, sum by (client_name) (increase(openmemory_requests_total{operation=~"create|update|delete"}[${duration}])))`)
...
export async function getTopReader(duration: string = '15m'): Promise<{ name: string, count: number }> {
  // Get client with most read requests in the given duration
  const result = await query(`topk(1, sum by (client_name) (increase(openmemory_requests_total{operation="read"}[${duration}])))`)
...
export async function getLastWriter(duration: string = '15m'): Promise<{ name: string, timestamp: number }> {
  try {
    const result = await query(`topk(1, sum by (client_name) (increase(openmemory_requests_total{operation=~"create|update|delete"}[${duration}])))`)
    if (result.data.result.length > 0 && result.data.result[0].value) {
      ...
      if (count > 0) {
        return {
          ...
          // We don't have exact timestamp from counter increase, so return 0 to avoid misleading "Just now"
          timestamp: 0
        }
      }
    }
...
export async function getLastReader(duration: string = '15m'): Promise<{ name: string, timestamp: number }> {
  try {
    const result = await query(`topk(1, sum by (client_name) (increase(openmemory_requests_total{operation="read"}[${duration}])))`)
    if (result.data.result.length > 0 && result.data.result[0].value) {
      const count = parseFloat(result.data.result[0].value[1])
