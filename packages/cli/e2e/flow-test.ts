...
const LOCAL_API_KEY = process.env.LOCAL_API_KEY || process.env.OM_API_KEY || "";
const RPI_API_KEY = process.env.RPI_API_KEY || "";
...
async function main() {
  console.log(
    "🚀 MCP Flow Test: write_local → backup → restore_rpi → verify\n",
  );
  ...
  try {
    // Initialize
    const initRes: any = await mcpPost(
      LOCAL_MCP,
      LOCAL_API_KEY,
      RPC(
        "initialize",
    ...
    // Store
    const storeRes: any = await mcpPost(
      LOCAL_MCP,
      LOCAL_API_KEY,
      RPC(
        "tools/call",
  ...
  try {
    ...
    execSync(
      `scp -i ${sshKey} "${backupPath}" ${rpiHost}:~/.cybermem/data/openmemory.sqlite`,
      { stdio: "pipe" },
    );

    console.log("   🔧 Fixing permissions...");
    execSync(
      `ssh -i ${sshKey} ${rpiHost} 'chmod 666 ~/.cybermem/data/openmemory.sqlite'`,
      { stdio: "pipe" },
    );
  ...
  try {
    ...
    if (getPayload.content === testContent) {
      ...
      console.log(
        `   📄 Content matches: "${getPayload.content.substring(0, 50)}..."`,
      );
    } else {
      throw new Error(
        `Content mismatch: expected "${testContent}", got "${getPayload.content}"`,
      );
    }
  } catch (e: any) {
