...
export async function init(options: any) {
  ...
  try {
    ...
    if (target === "local") {
      ...
      console.log(
        `  1. Open ${chalk.underline("http://localhost:3000/client-connect")} to connect your MCP clients`,
      );
      console.log(
        `  2. Default password: ${chalk.bold("admin")} (you'll be prompted to change it)`,
      );
      console.log("");
      console.log(
        chalk.dim(
