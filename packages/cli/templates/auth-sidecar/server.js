...
async function loadSecret() {
  try {
    ...
    if (!cachedToken && fs.existsSync(FALLBACK_TOKEN_PATH)) {
      cachedToken = fs.readFileSync(FALLBACK_TOKEN_PATH, "utf8").trim();
    ...
    if (!cachedToken) {
      console.warn(
        "⚠️  NO TOKEN FOUND - Auto-generating security token...",
      );
      ...
      try {
        ...
        await storeTokenInDatabase(cachedToken);
      } catch (err) {
        console.error(
          "Failed to persist auto-generated token:",
          err.message,
        );
        console.error(
      ...
      console.log("\n" + "=".repeat(70));
      console.log("🔐 AUTO-GENERATED SECURITY TOKEN (SAVE THIS!):");
      console.log(`   ${cachedToken}`);
      console.log("");
      console.log("This token was automatically generated because no token");
      console.log("was found. Save this securely - you'll need it to configure");
      console.log("MCP clients. You can also retrieve it from Dashboard Settings.");
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === "/health") {
    ...
    res.end(JSON.stringify({ status: "ok", mode: "token-auth", tokenSource: isAutoGenerated ? "auto-generated" : "configured" }));
    return;
  }

  // Token info endpoint (only accessible from localhost for security)
  if (req.url === "/token-info" && isLocalRequest(req)) {
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({
      isAutoGenerated,
      token: isAutoGenerated ? cachedToken : "***hidden***",
      message: isAutoGenerated 
        ? "Token was auto-generated. Please save it securely and configure it in your deployment."
